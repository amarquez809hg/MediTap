...
class PatientViewSet(BaseViewSet):
    queryset = models.Patient.objects.all().order_by("-created_at")
    serializer_class = serializers.PatientSerializer

class HospitalViewSet(viewsets.ModelViewSet):
    queryset = models.Hospital.objects.all()
    serializer_class = serializers.HospitalSerializer
    permission_classes = [permissions.IsAuthenticated, IntakeEditorWritePermission]

class IncidentViewSet(BaseViewSet):
    queryset = models.Incident.objects.select_related("patient", "hospital").all().order_by("-occurred_at")
...
class PatientChronicDiseaseViewSet(BaseViewSet):
    queryset = models.PatientChronicDisease.objects.select_related("patient","disease","medication").all()
...
class PatientLabPanelViewSet(viewsets.ModelViewSet):
    ...
    Creates/updates/deletes require superuser, Keycloak meditap-record-editor, or staff elevation.
    """
    ...
    def get_queryset(self):
        ...
        if self.action == "list":
            ...
            return qs.none()
        # retrieve / partial_update / update / destroy — no ?patient= on the URL
