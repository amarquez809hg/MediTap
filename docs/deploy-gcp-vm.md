...
| **`ERR_SSL_PROTOCOL_ERROR`** on `https://IP:8100` | Often **no SNI for raw IP** + Caddy had no cert to match. **Pull latest** `Caddyfile.public` (uses `default_sni` from `MEDITAP_PUBLIC_HOST`) and pass **`MEDITAP_PUBLIC_HOST`** into the **caddy** service in `docker-compose.override.yml` (see example file). Ensure `.env` has the same IP, then `docker compose up -d --build` **caddy**. If it still fails, confirm **only Caddy** publishes `8100` (`docker ps`). |
