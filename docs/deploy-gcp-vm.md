...
> **Note (2026):** The default MediTap stack no longer includes Keycloak; authentication is **Django + SimpleJWT**. The sections below still describe a **Keycloak-era** HTTPS layout (ports 8081, `keycloak-config`, etc.). For current Compose, use `docker/docker-compose.yml` and `docker-compose.override.public-https.example.yml` (SPA + API only) and ignore Keycloak-specific steps until this document is fully revised.
...
| SPA loads but API calls fail / empty data | Devtools → Network: API URL must match scheme/host (`https://IP:8080` with Caddy). `docker compose ps` should show **caddy** when using the override. If responses are **400** with an HTML body (“server error (400); check API logs…”), it is often **Django `DisallowedHost`**: the `Host` header is your public IP but **`ALLOWED_HOSTS`** did not include it. Set **`MEDITAP_PUBLIC_HOST`** in **`docker/.env`** (same IP you use in the browser); current **`medapp/settings.py`** appends that host automatically, then **`docker compose up -d --force-recreate backend`**. |
...
| **`ERR_SSL_PROTOCOL_ERROR`** on **`http://…nip.io:8081`** after **Sign in with Google** | Keycloak sent **`http://`** but Caddy on **8081** is **HTTPS only**. In realm **meditap** set **Require SSL** to **External requests** (or run `keycloak-config` after pull so `sslRequired=EXTERNAL` when `MEDITAP_PUBLIC_SCHEME=https`). Use **`https://`** for nip Keycloak URLs. Ensure Caddy’s TLS covers the nip hostname on **8081** if you use nip (see Caddyfile / host list). |
...
| **`keycloak-config` stuck on “Waiting for Keycloak…”** and **`auth`** logs show **`invalid_user_credentials`** for **`admin`** | The Keycloak **Postgres volume** already has a **different** master admin password than **`admin`**. Set **`KEYCLOAK_ADMIN_PASSWORD`** (and **`KEYCLOAK_ADMIN`** if not `admin`) in **`docker/.env`** to the password that works in **https://…:8081/admin**, then `docker compose run --rm keycloak-config`. Or **reset** Keycloak data (destructive): remove the **`keycloak_pg_data`** volume and recreate **`auth`** so bootstrap uses defaults again (see `docker compose` volume name via `docker volume ls`). |
