...
export function loadTab14LegacyFromLocalStorage(): Tab14LoadResult | null {
  ...
  return {
    ...
    patient: {
      ...
      systolicBp: String(patient.systolicBp ?? '').trim(),
      diastolicBp: String(patient.diastolicBp ?? '').trim(),
      heartRate: String(patient.heartRate ?? '').trim(),
      temperatureF: String(patient.temperatureF ?? '').trim(),
      temperatureC: String(patient.temperatureC ?? '').trim(),
